...
expe_root_dir = '/home1/namle/nist-sre10/'
task_dir = '/home1/namle/nist-sre10/task'
i4U_dir = '/home1/namle/nist-sre10/Sph_MetaData'
...
scoring = ['cosine', 'mahalanobis', '2cov',
           'plda']  # list of scoring to run on the task, could be 'cosine', 'mahalanobis', '2cov' or 'plda'
...
if train:
    ...
    enroll_stat.accumulate_stat(ubm=ubm, feature_server=fs, seg_indices=range(enroll_stat.segset.shape[0]),
                                numThread=nbThread)
    enroll_stat.save('data/stat_sre10_coreX-coreX_m_enroll_bnf.h5')

    nap_stat = sidekit.StatServer(nap_idmap, ubm)
    nap_stat.accumulate_stat(ubm=ubm, feature_server=fs, seg_indices=range(nap_stat.segset.shape[0]),
                             numThread=nbThread)
    ...
    test_stat.accumulate_stat(ubm=ubm, feature_server=fs, seg_indices=range(test_stat.segset.shape[0]),
                              numThread=nbThread)
    test_stat.save('data/stat_sre10_coreX-coreX_m_test_bnf.h5')
...
if plot:
    ...
    dp.set_system_from_scores(scores_cos_lda_wcnn, keysX[4], sys_name='Cosine WCCN LDA')
    ...
    dp.set_system_from_scores(scores_plda_efr1, keysX[4], sys_name='PLDA EFR')
